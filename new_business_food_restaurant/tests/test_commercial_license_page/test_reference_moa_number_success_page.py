...
from new_business_food_restaurant.pages.CommercialLicense.MOAReferenceNumberPage import MOAReferenceNumberPage
from new_business_food_restaurant.pages.CommercialLicense.SummaryDocumentPage import SummaryDocumentPage


class TestReferenceMOANumberSuccessPage:

    @pytest.mark.smoke
    def test_reference_moa_number_success_page(self):
        ...
        with allure.step("Select 'Create MOA' button"):
            moa = MOAPage()
            moa.click_reference_number_btn()

        """Reference MoA Number Page"""

        with allure.step("Fill reference number in text field"):
            reference_number = MOAReferenceNumberPage()
            reference_number.fill_reference_number()

        with allure.step("Select 'Submit' button"):
            reference_number.click_btn_submit()
