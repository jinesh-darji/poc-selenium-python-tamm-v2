...
from new_business_food_restaurant.pages.CommercialLicense.MOAUploadPage import MOAUploadPage
from new_business_food_restaurant.pages.CommercialLicense.SummaryDocumentPage import SummaryDocumentPage


class TestUploadMOASuccessPage:

    @pytest.mark.smoke
    def test_upload_moa_success_page(self):
        ...
        with allure.step("Select 'Create MOA' button"):
            moa = MOAPage()
            moa.click_upload_btn()

        """Upload MoA Page"""

        with allure.step("File Upload"):
            upload = MOAUploadPage()
            upload.upload_document_pdf("sample.pdf")

        with allure.step("Select 'Submit' button"):
            upload = MOAUploadPage()
            upload.click_btn_submit()
