...
class AddPartnerForm(BaseForm):
    form_name = "Add Partner Form"
    locator_reader = LocatorReader(form_name)
    page_element = Label(locator_reader, "add_partner_locator")
    ...
    btn_calendar_icon = Button(locator_reader, "btn_calendar_icon")

    def __init__(self):
        super(AddPartnerForm, self).__init__(self.page_element)

    def fill_name_in_english(self):
        for element in self.txb_name_in_english.get_elements():
            element.send_keys(DataReader.get_data("add_partner.txb_name_in_english"))

    def fill_name_in_arabic(self):
        for element in self.txb_name_in_arabic.get_elements():
            element.send_keys(DataReader.get_data("add_partner.txb_name_in_arabic"))

    def set_dob_date(self):
        self.dob_datepicker.set_today_date()

    def click_calendar_icon(self):
        self.btn_calendar_icon.click()

    def fill_emirati_id(self):
        for element in self.txb_emirati_id.get_elements():
            element.send_keys(DataReader.get_data("add_partner.txb_emirati_id"))

    def fill_email(self):
        for element in self.txb_email.get_elements():
            element.send_keys(DataReader.get_data("add_partner.txb_email"))

    def fill_phone_number(self):
        for element in self.txb_phone_number.get_elements():
            element.send_keys(DataReader.get_data("add_partner.txb_phone_number"))

    def fill_emirates(self):
        for element in self.txb_emirates.get_elements():
            element.send_keys(DataReader.get_data("add_partner.txb_emirates"))

    def fill_city(self):
        for element in self.txb_city.get_elements():
            element.send_keys(DataReader.get_data("add_partner.txb_city"))

    def fill_area(self):
        for element in self.txb_area.get_elements():
            element.send_keys(DataReader.get_data("add_partner.txb_area"))

    def fill_street(self):
        for element in self.txb_street.get_elements():
            element.send_keys(DataReader.get_data("add_partner.txb_street"))

    def fill_building(self):
        for element in self.txb_building.get_elements():
            element.send_keys(DataReader.get_data("add_partner.txb_building"))
